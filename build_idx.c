...
int main (int argc, char **argv) {
  ...
  while (!(feof(fp))) {
    ...
    while(read_index(fkhs, key_hash, &i) == -1) {
      key_hash++;
      if (key_hash == capacity) { // roll back to 0
        key_hash = 0;
      }
    }
    write_index(fkhs, i, key_hash);
    while(read_index(fvhs, value_hash, &i) == -1) {
      value_hash++;
      if (value_hash == capacity) { // roll back to 0
        value_hash = 0;
      }
    }
